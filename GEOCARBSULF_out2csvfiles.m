...
if (resampleN==1)
    GEOCARB_header = {'age','failed runs','CO2','O2'};
    ...
    if (strcmp(loop_parameters,'TRUE'))
        ...
        % admittedly a lazy implementation of this, but works and not very
        % resource intensive relative to the rest of the code.
        GEOCARB_header = {'age','failed runs','CO2'};
        for mm = 1:length(percentile_values)
            GEOCARB_header{end+1} = ['CO2 ',num2str(100*percentile_values(mm)),'%'];
        end
        GEOCARB_header{end+1} = 'O2';
        for mm = 1:length(percentile_values)
            GEOCARB_header{end+1} = ['O2 ',num2str(100*percentile_values(mm)),'%'];
        end        
